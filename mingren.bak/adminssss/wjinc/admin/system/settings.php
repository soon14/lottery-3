...
<input type="hidden" value="<?=$this->user['username']?>" />
	...
	<form name="system_install" action="/qq836651666.php/system/updateSettings" method="post" target="ajax" call="sysSettings" onajax="sysSettingsBefor">
	<table class="tablesorter left" cellspacing="0" width="100%">
		<tbody>
			...
			<tr>
				<td>滚动公告</td>
				<td>
					<textarea name="webGG" cols="100" rows="5"><?=$this->settings['webGG']?></textarea>
				</td>
			</tr>
            <tr>
				<td>总投注开关</td>
			...
			<tr>
				<td>是否开通 "分" 模式</td>
				<td>
					<label><input type="radio" value="1" name="fenmosi" <?=$this->iff($this->settings['fenmosi'],'checked="checked"')?>/>开启</label>
					<label><input type="radio" value="0" name="fenmosi" <?=$this->iff(!$this->settings['fenmosi'],'checked="checked"')?>/>关闭</label>
					<br />
				</td>
			</tr>
			<tr>
				<td>余额宝存款利率</td>
				<td>
					<label><input type="text" name="depositll"  value="<?=$this->settings['depositll']?>" style="width:60px"/>%&nbsp;&nbsp;例如设置0.1,计算则为余额宝*0.1*0.01*时间</label>
				</td>
			</tr>
			<tr>
				<td>网站注册人数</td>
				<td>
					注册人数:<input type="text" class="totalcount" value="<?=$this->settings['totalcount']?>" name="totalcount"/>
                	奖金:<input type="text" class="totalcoin" value="<?=$this->settings['totalcoin']?>" name="totalcoin"/>
				</td>
			</tr>
			<tr>
				<td>系统彩种利润</td>
				<td>
					...
					<label><input type="radio" value="1" name="LiRunLv" <?=$this->iff(!$this->settings['LiRunLv'],'checked="checked"')?>/>低</label>
				</td>
			</tr>
			<tr>
				<td>会员上榜最低中奖金额</td>
			...
			<tr>
				<td>虚拟上榜会员昵称</td>
				<td>
					<textarea name="paihangsjnr" cols="100" rows="3"><?=$this->settings['paihangsjnr']?></textarea>&nbsp&nbsp&nbsp*最好不少于20个，请用"|"隔开，用于参合到真实数据中
				</td>
			</tr>
			<tr>
				<td>虚拟上榜中奖金额</td>
				<td>
					<textarea name="paihangsjje" cols="100" rows="3"><?=$this->settings['paihangsjje']?></textarea>&nbsp&nbsp&nbsp*最好不少于20个，请用"|"隔开，用于参合到真实数据中
				</td>
			</tr>
			<tr>
				<td>返点最大值</td>
			...
			<tr>
				<td>清理账号规则</td>
				<td>账户金额低于&nbsp;<input type="text" class="textWid1" value="<?=$this->settings['clearMemberCoin']?>" name="clearMemberCoin" id="clearMemberCoin"/>元，&nbsp;且&nbsp;<input type="text" class="textWid1" value="<?=$this->settings['clearMemberDate']?>" name="clearMemberDate" id="clearMemberDate"/> &nbsp;天未登录&nbsp;&nbsp;<a method="post" target="ajax" onajax="clearUsersBefor" call="clearDataSuccess" title="数据清除不可修复，是否继续！" dataType="json" id="alt_btn3" href="/qq836651666.php/System/clearUser">清理</a></td>
			</tr>
			<tr>
				<td>清理数据</td>
				<td>清除当前 <input type="date" readonly="readonly" id="clearData" /> 以前的投注、帐变、管理员日志、会员登录日志、提现、充值、采集记录数据&nbsp;&nbsp;<a method="post" target="ajax" onajax="clearDataBefor" call="clearDataSuccess" title="数据清除不可修复，是否继续！" dataType="json" id="alt_btn3" href="/qq836651666.php/System/clearData">清理</a></td>
			</tr>
			<tr>
				<td>清理数据 2</td>
				<td>仅清除当前 <input type="date" readonly="readonly" id="clearData2" /> 以前的采集记录数据&nbsp;&nbsp;<a method="post" target="ajax" onajax="clearDataBefor2" call="clearDataSuccess2" title="采集记录数据清除不可修复，是否继续！" dataType="json" id="alt_btn3" href="/qq836651666.php/System/clearData2">清理</a></td>
			</tr>
			<tr>
				<td>赠送活动</td>
			...
			<tr>
				<td>积分规则</td>
				<td>
					<textarea name="scoreRule" cols="100" rows="2"><?=$this->settings['scoreRule']?></textarea>
				</td>
			</tr>
            <tr>
				<td>客服状态</td>
			...
			<tr>
				<td>客服链接</td>
				<td>
					<textarea name="kefuGG" cols="100" rows="2"><?=$this->settings['kefuGG']?></textarea>
				</td>
			</tr>
		</tbody>
	</table>
