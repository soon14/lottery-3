...
<tbody id="nav01">
<?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ if($data['data']) foreach($data['data'] as $var){ if($var['state']){$amount+=$var['rechargeAmount'];}?>
    <tr>
        <td><?=$var['uid']?></td>
        <td><?=$var['username']?></td>
        ...
        <td><a href="<?=$bank[$var['BankId']]['home']?>" title="银行帐号：<?=$bank[$var['BankId']]['account']?>，开户名：<?=$bank[$var['BankId']]['username']?>" target="_blank"><?=$bank[$var['BankId']]['name']?></a></td>
        <td><?=$this->iff($var['state'], '充值成功', '正在充值')?></td>
        <td><?=$var['info']?></td>
        <td><?=date('Y-m-d H:i:s', $var['actionTime'])?></td>
        <td>
            <?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ if(!$var['state']){ ?>
            <a href="/qq836651666.php/business/rechargeActionModal/<?=$var['id']?>" target="modal"  width="420" title="编辑用户" modal="true" button="确定:dataAddCode|取消:defaultCloseModal">到帐处理</a>
            <a href="/qq836651666.php/business/rechargeDelete/<?=$var['id']?>" target="ajax" dataType="json" call="defaultAjaxLink">删除</a>
            <?php
