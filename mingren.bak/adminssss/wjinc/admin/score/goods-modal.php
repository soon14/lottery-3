...
<body>
<form name="score_addGoods" action="/qq836651666.php/Score/updateGoods" enctype="multipart/form-data" method="POST">
<?php
...
?>
    ...
    <tbody>
        ...
        <tr> 
            <td>时间</td> 
            <td>从 <input type="date"  name="startTime" style="width:75px;" value="<?=date('Y-m-d H:i:s',$goods['startTime'])?>"/> 到  <input type="date" name="stopTime" style="width:75px;" value="<?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ if($goods['stopTime']){echo date('Y-m-d H:i:s',$goods['stopTime']);}else{echo '0';}?>"/><span class="spn6">0为不过期</span></td>
        </tr>
        <tr> 
            <td>缩略图</td> 
        ...
        <tr> 
            <td>状态</td> 
            <td>
                <label><input type="radio" value="1" name="enable" <?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ if($goods["enable"]==1){?> checked='checked'<?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ }?>/>开启</label>
                <label><input type="radio" value="0" name="enable" <?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ if($goods["enable"]==0){?> checked='checked'<?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ }?>/>关闭</label>
            </td> 
        <tr> 
    </tbody> 
    </table>
