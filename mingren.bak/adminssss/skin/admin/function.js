...
function load(url, options){
	...
	options.url='/qq836651666.php/'+options.url||'';
...
function checkStatus(xhr){//{{{
	...
	if(typeof xhr.getResponseHeader('x-not-login') == 'undefined'){
		top.location='/qq1655707002.php/user/login';
		return false;
	}
	// 检查是否跳转
...
function rechargModal(){
	$.get('/qq836651666.php/' + 'business/rechargeModal', function(html){
		...
		$(html).dialog({
			...
			buttons:{
				"确定充值":function(event, ui){
					...
					$.ajax( '/qq836651666.php/' + 'business/rechargeAction/' + uid + '/' + amount+ '/' + userRid, {
						dataType:'json',
						error:defaultError,
...
function manageAddManagerModal(){
	$.get('/qq836651666.php/' + 'manage/addManagerModal', function(html){
...
function payAddModal(){
	$.get('/qq836651666.php/' + 'pays/addpaymodal', function(html){
...
function sysEditBank(id){
	...
	$('<div class="bank-modal"><iframe frameborder="0" src="/qq836651666.php/system/bankModal/'+(id||'')+'" style="width:480px;height:230px;"></iframe></div>')
	.dialog({
		title:'编辑银行',
		width:500,
		buttons:{
...
function memberEditBank(id){
	$('<div class="bank-modals"><iframe frameborder="0" src="/qq836651666.php/system/bankModal2/'+(id||'')+'" style="width:480px;height:230px;"></iframe></div>')
	.dialog({
		title:'编辑银行',
		width:500,
		buttons:{
...
function scoreEditGoods(id){
	...
	$('<div class="goods-modal"><iframe frameborder="0" src="/qq836651666.php/Score/goodsModal/'+(id||'')+'" style="width:580px;height:470px;"></iframe></div>')
	.dialog({
		title:'编辑商品',
		width:600,
		buttons:{
...
function fabuxiaoxi(){
	$.get('/qq836651666.php/' + 'member/fabuxiaoxi', function(html){
