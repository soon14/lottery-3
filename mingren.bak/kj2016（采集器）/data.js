	...
	http.request(option, function(res){
		...
		res.on("end", function(){
			try{
				...
				try{
					if(data.type=='14' || data.type=='26' || data.type=='5' || data.type=='30'){
						liRunData(data, conf);
					}else{
...
function liRunData(data, conf){
	...
	getLiRunLv();
	var client=createMySQLClient();
	client.query("select actionNum,playedId,actionData,weiShu,mode,beiShu,bonusProp from ssc_bets where type=? and actionNo=? and isDelete=0 and lotteryNo=''", [data.type, data.number], function(err, bets){
		if(err){
			log("读取投注出错："+err);
		}else{
...
function calcJ(data, flag){
	var client=createMySQLClient();
	sql="select id,playedId,actionData,weiShu from ssc_bets where type=? and actionNo=? and isDelete=0";
	...
	client.query(sql, [data.type, data.number], function(err, bets){
		if(err){
			...
			bets.forEach(function(bet){
				...
				sqls.push(client.format(sql, [bet.id, zjCount, data.data, 'QQ:446284379-'+encrypt_key]));
