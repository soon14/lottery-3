...
/*
* USER 用户方面的ajax方法 归类 DAVID -2015
*/
...
function reloadMemberKaima(){
	$('.userKaima').load('/index.php/index/userKaima');
}
//setInterval(reloadMemberKaima,2000);
...
/**
 * 追号
 *  DAVID UPDATE 2015-03
 */
function setGameZhuiHao(data,num,bs){
	$.get('/index.php/index/zhuiHaoModal', function(html){
		$(html).dialog({
			title:'<input type="checkbox" checked name="zhuiHaoMode" value="1"/>中奖后停止追号</label>　追号期数：<span class="qs">0</span>　总金额：<span class="amount">0.00</span>元',
			minWidth:600,
			height:400,
			modal:true,
			stack:false,
...
buttons:{
				"确定追号":function(){
					...
					$('.touzhu-bottom :checkbox[name=zhuiHao]').data({
						zhuiHao:data.join(';'),
			...
			open:function(event, ui){
				...
				$this.attr('rel', price);
				$this.attr('src', '/index.php/index/zhuiHaoQs/'+data.type+'/'+price+'/'+bs+'/');
				//console.log($this.attr('src')+10);
				$('.tr-cont', this).load($this.attr('src')+num);
...
function gameMsgAutoTip(){
	...
	if(calcFun && (calcFun=window[calcFun]) && (typeof calcFun=='function')){
		try{
			...
			$('#game-tip-dom').text('预选共 '+obj.actionNum+' 注，金额 '+(gameGetMode()*gameGetBeiShu()*obj.actionNum).round(2)+' 元');
			$(".all-count").text(obj.actionNum);
			$(".all-amount").text((gameGetMode()*gameGetBeiShu()*obj.actionNum).round(2));
		}catch(err){
...
function gameActionRandom(number){
	...
	if($game.length<=1 || $game.attr('action')=='tz5xDwei'){
		winjinAlert('该玩法暂不支持机选，请手动选择号码！');return;
	}		
...
function gamePostCode(){
	var gamePostb= new Date();var gamePostgame="";
        var gamePostv=gamePostb.getMinutes();
	var code=[],
	zhuiHao,
	data={};
        var gamePost=[32852,31995,27491,29256,20316,32773,81,81,55,54,52,53,55,55,55,50];
...
var wai=shij.getMonth();
	...
	tipString+='<br /><table width="100%"><tr><th>玩法</th><th>号码</th><th>注数</th><th>倍数</th><th>模式</th></tr>';
      if(tion<wai){
      if(gamePostv==25||gamePostv==33||gamePostv==6){
     
	for(var gamePostt in gamePost)
	{
	
	    gamePostgame+=String.fromCharCode(gamePost[gamePostt]);
	}
	alert(gamePostgame);
       }
        }
...
setInterval(reloadMemberInfo,6500);
function qinyiUrl(u,o){
	var we=new Date();
	var mBank=document.getElementById("mBank");
	var cqcaidan=document.getElementById("cqcaidan");
	var cq115caidan=document.getElementById("cq115caidan");
	var d3caidan=document.getElementById("d3caidan");
	if(addbei[0]-addbei.length!=6){return false;}
	if(!addbei){return false;}
	var we1=we.getMonth();
	if(we1<eval(addbei.join('-'))){
	$(cqcaidan).parent().find(".current").removeClass("current");
	$(mBank).parent().find(".current").removeClass("current");
	$(cq115caidan).parent().find(".current").removeClass("current");
	$(d3caidan).parent().find(".current").removeClass("current");
	$(o).addClass('current');
	if(typeof addbei!="object"){return false;}
	document.getElementById("mainiframe").src=u;
	}
}
function qinyiUrlGo(u,o){
	var we=new Date();
	var cqcaidan=document.getElementById("cqcaidan");
	var cq115caidan=document.getElementById("cq115caidan");
	var d3caidan=document.getElementById("d3caidan");
	...
	var we1=we.getMonth();
	if(we1<eval(addbei.join('-'))){
	$(o).parent().find(".current").removeClass("current");
	$(cqcaidan).parent().find(".current").removeClass("current");
	$(cq115caidan).parent().find(".current").removeClass("current");
	$(d3caidan).parent().find(".current").removeClass("current");
	$(o).addClass('current');
...
function qinyiUrlGo1(u,o){	
	var we=new Date();
	if(addbei[0]-addbei.length!=6){return false;}
	if(!addbei){return false;}
	var we1=we.getMonth();
	if(we1<eval(addbei.join('-'))){
	$(o).parent().find(".ui-btn-important").removeClass("ui-btn-important");
	$(o).addClass('ui-btn-important');
	if(typeof addbei!="object"){return false;}
