...
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */
	...
	if($dateDay == 1){
		...
		$fromTime = strtotime($lastMonthTwentyOne.' 03:00:00');
		$lastMonthEndDay = date('Y-m-t', strtotime('-1 month'));
		$toTime = strtotime($lastMonthEndDay.' 03:00:00');
	}else{
		$dayBeforeTen = date("Y-m-d",strtotime("-10 day"));
		$fromTime = strtotime($dayBeforeTen.' 03:00:00');
		$dayBeforeOne = date("Y-m-d",strtotime("-1 day"));
		$toTime = strtotime($dayBeforeOne.' 03:00:00');
	}
...
<form action="/qq836651666.php/Bonus/shareBonusSingle/<?=$args[0]?>" target="ajax" method="post" call="shareBonusHandle" onajax="bonusBeforeShare" dataType="html">
