...
<input type="hidden" value="<?=$this->user['username']?>" />
	<header>
		...
		<form class="submit_link wz" action="/qq836651666.php/data/index/<?=$this->type?>" target="ajax" call="defaultSearch" dataType="html">
...
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */
			...
			<tr>
				...
				<td><?=$this->iff($data['data'], '<font color="red">已开奖</font>', '未开奖')?></td>
				...
				<td>
					<a href="/qq836651666.php/data/add/<?=$this->type?>/<?=$var['actionNo']?>/<?=$dateString.$var['actionTime']?>" target="modal" width="340" title="添加开奖号码" modal="true" button="确定:dataAddCode|取消:defaultCloseModal">添加</a>
					<?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ if($data['data']){ ?>
					<a href="/qq836651666.php/data/kj" target="ajax" data-type="<?=$typeInfo['id']?>" data-number="<?=$data['number']?>" data-time="<?=$dateString.$var['actionTime']?>" data-data="<?=$data['data']?>" onajax="setKjData" call="setKj" title="重新对没有开奖的投注开奖">开奖</a>
					<?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ } ?>
                    
				</td>
                <td>
                <a href="/qq836651666.php/data/back/<?=$this->type?>/<?=$var['actionNo']?>/<?=$dateString.$var['actionTime']?>"  target="modal" width="340" title="对未开奖期号进行投注退款"  modal="true" button="确定:dataAddCode|取消:defaultCloseModal" style="color:#F00;">退款</a>
                </td>
			</tr>
			<?php
/* 此程序来自家园网络QQ836651666 采集修复 程序修改 仅供研究学习之用 禁止用于商业非法用途 */ } ?>
