...
下行地址：<input name="url" type="text" value="<?php echo(isset($_POST['url']) ? $_POST['url'] : "http://localhost/shunfoo/callback/pay_bank_callback.php");?>" size="80"/>
<p><strong>请求协议参数：</strong><br>
</p>
<table border="1" cellspacing="0" cellpadding="0" width="552">
  ...
  <tr>
    ...
    <td width="408" valign="top"><p><input type="text" name="opstate"  value="<?php echo($_POST['opstate']);?>"/><br>操作结果状态。<br>
      ...
      -4 卡已经使用（卡在提交到顺付之前已经被使用） <br>
      -5 失败(网络原因、具体原因不明确等)</p></td>
  </tr>
  <tr>
    <td valign="top">ovalue</td>
    <td valign="top"><input type="text" name="ovalue"  value="<?php echo($_POST['ovalue']);?>"/>
      <br>
opstate=-2或者-3时表示的值，单位元(注：现只提供正确的骏网卡实际面值，其他卡值为0或者无效。为了精确性，该值可能带有4位小数)</td>
  </tr>
  <tr>
    <td valign="top">shunfooorderid</td>
    <td valign="top"><input type="text" name="shunfooorderid"  value="<?php echo($_POST['shunfooorderid']);?>"/>
      <br />
      此次卡消耗过程中顺付系统的订单Id。为保持和以前版本兼容，该值不加入返回结果签名验证。</td>
  </tr>
  <tr>
    <td valign="top">shunfootime</td>
    <td valign="top"><input type="text" name="shunfootime"  value="<?php echo($_POST['shunfootime']);?>"/>
      <br />
      此次卡消耗过程中顺付系统的订单结束时间。格式为<br />
        年/月/日 时：分：秒，如2015/04/05 21:50:58。<br />
